...
namespace DigitalRuby.WeatherMaker
{
    public class WeatherMakerDayNightCycleScript : MonoBehaviour
    {
        ...
        [Range(0.0f, 1.0f)]
        [Tooltip("The maximum sun shadow strength")]
        public float MaxSunShadowStrength = 0.8f;
        ...
        private void UpdateSunPosition()
        {
            if (lastTimeOfDay != TimeOfDay)
            {
                ...
                else
                {
                    ...
                    bool horizonShadowBlock = (dot <= 0.0f);
                    ...
                    if (dot <= SunDotFadeThreshold && dot >= SunDotDisableThreshold)
                    {
                        ...
                        {
                            Sun.intensity = Mathf.Lerp(0.0f, 1.0f, (dot - SunDotDisableThreshold) / range);
                        }
                        else
                        {
                            Sun.intensity = Mathf.Lerp(1.0f, 0.0f, (SunDotFadeThreshold - dot) / range);
                        }
                        horizonShadowBlock = true;
                    }
                    else
                    {
                        Sun.intensity = (Sun.enabled ? 1.0f : 0.0f);
                        horizonShadowBlock = false;
                    }
                    ...
                    Sun.shadowStrength = (horizonShadowBlock ? MaxSunShadowStrength : Mathf.Lerp(0.0f, MaxSunShadowStrength, Mathf.Min(1.0f, Sun.intensity * Sun.intensity)));
